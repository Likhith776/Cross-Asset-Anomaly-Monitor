...
[`docs/backtest_report.md`](docs/backtest_report.md) is a committed,
regenerable evaluation of every detector: precision / recall / F1 per
detector against planted anomalies (single-symbol spikes/dips/vol
bursts at varying magnitudes, plus joint-flip scenarios only the
multivariate detector can catch), a false-positive rate from clean
replays, and a recall-by-magnitude chart
([`docs/backtest_report.png`](docs/backtest_report.png)).

It is generated by `scripts/run_backtest_report.py` from the same
harness the tests use (`src/backtest/harness.py` — `replay_panel`,
`score_trial`) over deterministic synthetic panels: fixed seeds, no
network, so the report changes exactly when detector code changes.
Regenerate it manually after touching any detector
(`python scripts/run_backtest_report.py`), or re-run it from Actions
via the **BACKTEST-REPORT** workflow (manual dispatch; commits the
report only when it actually changed). It never runs on push or PR and
never blocks CI.

## LLM anomaly explanations (optional, off by default)

High-severity anomalies (score >= 0.7, i.e. high/critical) can carry a
1-2 sentence natural-language explanation of what the cross-asset data
shows — co-movement, correlation breaks, lead-lag, scheduled events,
volatility regime. Generated by a single Gemini API call over the
feature snapshot, strictly instructed to reason only from the data (no
news speculation).

- **Off by default and zero-risk without a key**: set `GEMINI_API_KEY=`
  in `.env` (generate at [Google AI Studio](https://aistudio.google.com/apikey))
  to enable; leave it empty and every surface works identically with
  the explanation stored as null.
- **Free tier only**: pinned to `gemini-2.5-flash` (listed free of
  charge on the Gemini API free tier; see
  [pricing](https://ai.google.dev/gemini-api/docs/pricing) — re-verify
  when updating `GEMINI_MODEL` in `src/detection/explain.py`).
- **Hard budget**: a process-wide sliding window caps calls at 6/hour
  and 30/day, enforced *before* any network call and consumed even for
  failed calls so retries can't storm the quota. The severity gate plus
  these caps keep even unattended scheduled paths far below free-tier
  limits. The gate/budget/threshold constants live at the top of
  `src/detection/explain.py`.
- **Never a dependency**: any failure (missing key, HTTP error, timeout,
  exhausted budget) stores the explanation as null; the anomaly record
  itself is always written first-class.
- The LLM never gates detection, never runs on low-severity anomalies,
  and tests are fully offline (an autouse fixture force-disables the
  feature suite-wide).
